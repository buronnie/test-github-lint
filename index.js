angular.module('Magellan').directive('concourseApplicationTree', function(
	...
	return {
		...
		controller: function($scope) {
			...
			// maintains the heights of the application tree with children expansion status
			var applicationHeights = {};

			// stores the additive scrollTop values for each application
			var applicationScrollTop = [];

			// need to set on scope for the link function to have access
			...
			$scope.delegateTreeClick = function($event) {
				...
				// detect if an application was clicked
			...
			function getApplicationStyles(app) {
				...
				return min > ITEM_HEIGHT ? {minHeight: min} : {};
			}
			...
			/**
			 * Determine if a max threshold for scrolling is hit to unbind the scroll spy
			 * @param top
			 * @return {boolean}
			 */
			...
			function renderApplicationsOnScreen(scrollTop) {
				...
				// use the last known scroll value if called without a scrollTop
			...
			function _initializeLazyRender(t) {
				...
				// compute our current application heights
				var scrollTop = 0;
				...
				// determine which applications should be visible initially
			...
			/**
			 * Updates the filter results for the screen reader
			 */
			function updateForScreenReader(numFilteredResults) {
				const pluralString = numFilteredResults + ' items found';
				const singularString = numFilteredResults + ' item found';

				const results =  (numFilteredResults === 1) ? singularString : pluralString;
			...
			concourseNavigatorService.onChangeVisibility(function(result) {
				...
				// render any apps that are not rendered
				_renderMissingApplications(apps, true).then(function() {

					// paint the new results
					for (var i = 0, iM = itemsVisibility.length; (i * FRAME_SIZE) < iM; i++) {
						(function(frameIndex) {
			...
			/**
			 * Given some apps, queue for rendering if not already rendered
			 * @param apps
			 * @return {*}
			 * @private
			 */
			...
			function _updateFrameVisibility(itemsVisibility, frameIndex, frameSize) {
				...
				for (var i = frameIndex * frameSize, iM = i + frameSize; i < iM; i++) {
					...
					// if undefined, we have reached the end
					if (!itemVisibility) {
						return;
					}
			...
			// innertText is not supported by FF44 (selenium env), instead it supports textContent
